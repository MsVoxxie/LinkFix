...
module.exports = {
	...
	async execute(client, message) {
		...
		// Define Regex
		const linkRegex = /https:\/\/bsky\.app\/profile\/([^\/]+)\/post\/([a-zA-Z0-9]{1,20})/gm;
		...
		for (const match of linkMatches) {
			...
			const finalLink = `https://cbsky.app/profile/${userId}/post/${linkId}`;

			// Format the message
			const formattedMessage = hyperlink(`Bsky • ${userId} - ${linkId}`, finalLink);
		...
		// Run the link fixer
		try {
			await linkFixer(message, originalMessage, messagesToSend, '<:bsky:1297323816787120209>');
		} catch (error) {
