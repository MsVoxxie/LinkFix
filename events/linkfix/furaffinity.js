...
module.exports = {
	...
	async execute(client, message) {
		...
		// Define Regex
		const linkRegex = /https:\/\/www\.furaffinity\.net\/view\/(\d+)\//gm;
		...
		for (const match of linkMatches) {
			...
			const finalLink = `https://xfuraffinity.net/view/${linkId}`;

			// Format the message
			const formattedMessage = hyperlink(`Furaffinity • ${linkId}`, finalLink);
